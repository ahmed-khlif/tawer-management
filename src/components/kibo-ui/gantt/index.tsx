...
> = memo(({ label, date, id, onRemove, onClick, className }) => {
  const gantt = useContext(GanttContext);
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  let columnWidth = columnWidthOverride ?? 50;

  if (!columnWidthOverride && range === "monthly") {
    columnWidth = 150;
  } else if (!columnWidthOverride && range === "quarterly") {
    columnWidth = 100;
  }
  ...
  useEffect(() => {
    const updateSidebarWidth = () => {
      if (typeof sidebarWidthOverride === "number") {
        setSidebarWidth(sidebarWidthOverride);
        return;
      }
    ...
    >
      <div
        ...
        style={{
          ...cssVariables,
          gridTemplateColumns:
            sidebarWidth > 0 ? "var(--gantt-sidebar-width) 1fr" : "1fr",
        }}
      >
        {children}
      </div>

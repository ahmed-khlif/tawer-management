...
import { ChatContent, ChatSidebar } from "@/components/pages-components/apps/chat";
...
async function getChats() {
  const data = await fs.readFile(
    path.join(process.cwd(), "src/app/[locale]/dashboard/(auth)/apps/chat/data/chats.json")
  );
...
async function getChatUser(id: number) {
  const data = await fs.readFile(
    path.join(process.cwd(), "src/app/[locale]/dashboard/(auth)/apps/chat/data/contacts.json")
  );

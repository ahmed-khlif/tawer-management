...
import { AgendaDaysToShow, CalendarEventType, EventItem, getAgendaEventsForDay } from "..";
...
export function AgendaView({ currentDate, events, onEventSelect }: AgendaViewProps) {
  const t = useTranslations("modules.events.calendar.views.agenda");
    ...
    <div className="border-border/70 border-t px-4">
      {!hasEvents ? (
        <div className="flex min-h-[70svh] flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-lg font-medium">{t("noEvents.title")}</h3>
          <p className="text-muted-foreground">{t("noEvents.description")}</p>
        </div>
      ) : (
        days.map((day) => {
